...
module.exports = class TaskController {
  ...
  // funcao que da update no status da tarefa
  static async toggleTaskStatus(req, res) {
    // recebemos o id por um form
    const id = req.body.id;

    // recebe o done e se for 0 troca para true,e caso nao seja 1 altera para false
    const task = {
  ...
  // funcao que acessa a rota em especifico de cada item
  static async taskIndividual(req, res) {
    // resgata a função pelo id
    const id = req.params.id;

    // recupera a tarefa em especifico pelo id
    const task = await Task.findOne({ raw: true, where: { id: id } });

    res.render("tasks/individual", { task });
  }
