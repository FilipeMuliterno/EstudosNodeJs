...
app.post("/books/insertbook", (req, res) => {
  ...
  // preparação para ficar mais segura a query
...
// cria uma rota que faz a exclusao no BD utilizando o id passado no form de exlusao na pagina book
app.post("/books/remove/:id", (req, res) => {
  // recupera o id da url
  const id = req.params.id;

  // query que faz a exclusao do BD
  const query = `DELETE FROM books WHERE id = ${id}`;

  // faz a exclusao utilizando a query acima no banco de dados
  pool.query(query, function (err) {
    if (err) {
      console.log(err);
    }
